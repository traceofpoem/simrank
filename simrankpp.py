...
#  simrankpp.py
#
#  Copyright 2020 liupeng <liupeng@southernfund.com>
#
...
def simrank_double_plus_bipartite(G, r=0.8, max_iter=100, eps=1e-4):
    ...
    def _calculate_spread(ns, is_lns=True):
        ...
        for n in ns:
            ...
            weights = np.asarray(list(nbr.values()))
    ...
    def _calculate_normalized_weight(ns, is_lns=True):
        for n in ns:
            ...
            denom_factor = np.sum(list(nbrs.values()))
...
def main(args):
    ...
    G = BipartiteGraph()
    import pandas as pd
    fname = r"D:\scripts\python\tf\_4sr_201201.csv"
    df = pd.read_csv(fname,encoding="gbk")
    for i in df.index:
        G.add_edge(df.loc[i,"real_author"],df.loc[i,"windcode"], 1.0)
    lns_sim, rns_sim = simrank_double_plus_bipartite(G,r=0.5,max_iter=5)

    print("sim")
    for node, index in G.get_lns_index().items():
        print("%d | %s" % (index, node))
    print(lns_sim)

    for node, index in G.get_rns_index().items():
        print("%d | %s" % (index, node))
    print(rns_sim)
    lns_sim_dict, rns_sim_dict = convert_sim_to_dict(G, lns_sim, rns_sim)
